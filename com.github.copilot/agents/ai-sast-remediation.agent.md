...
2. Select once, then parse one Endor verdict. With no supplied Finding UUID, resolve Project once, then select the target Finding with two server-side queries and no local parsing: first the grouped severity counts, then one single-row query for the selected UUID (both shapes below). Selection rule `severity_desc_uuid_asc_v1` is applied by the API, not by you: descend `CRITICAL`, `HIGH`, `MEDIUM`, `LOW`, `INFO` to the first band with a non-zero count, then take the lowest UUID in that band. Copy only the severity counts, the selected level, and the selected Finding UUID into model context. Fetch `spec.finding_metadata` and `spec.explanation` only for that selected Finding, then parse its Classification line, Verification Scorecard, Severity Scoring, Data Flow anchors, Exploit Reproduction, Remediation Guidance, and sibling-file hints. Never list the full inventory into context, run a model-written parser over rows, repeat the selection, or issue a separate count cross-check.
   ...
   - Selection query 1 of 2, grouped severity counts. Let `<AI_SAST_SCOPE>` be `context.type==CONTEXT_TYPE_MAIN and spec.project_uuid=="<PROJECT_UUID>" and spec.method=="SYSTEM_EVALUATION_METHOD_DEFINITION_AI_SAST"`, then run `endorctl agent api --agent-id ai-sast-remediation list -r Finding -n <namespace> --filter '<AI_SAST_SCOPE>' --group-aggregation-paths spec.level -o json`. The returned `group_response.groups` map is the authoritative `severity_counts`; no row values enter model context. `--group-aggregation-paths` cannot be combined with `--count`.
   - Selection query 2 of 2, the selected finding. For the highest band whose count is non-zero, run `endorctl agent api --agent-id ai-sast-remediation list -r Finding -n <namespace> --filter '<AI_SAST_SCOPE> and spec.level=="FINDING_LEVEL_<BAND>"' --field-mask "uuid,spec.level" --sort-path uuid --sort-order ascending --page-size 1 -o json`. The single returned `uuid` is the selected Finding. Report `selection_rule: severity_desc_uuid_asc_v1` and the tie count at the selected level from query 1. Use `--count` only in the separate availability-only evidence-check profile; never add another count here.
...
- `project-by-git`/evidence-check: `endorctl agent api --agent-id ai-sast-remediation list -r Project -n <namespace> --filter 'spec.git.full_name=="<owner/repo-lowercased>"' --page-size 2 --field-mask "uuid,meta.name,meta.parent_uuid,spec.git" -o json`
