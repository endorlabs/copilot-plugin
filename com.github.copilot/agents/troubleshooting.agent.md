...
Field extraction or summarization of that response does not
...
- Never merge stderr into the JSON stream with `2>&1`; it corrupts
...
- server-side projection flags such as `--field-mask`, `--count`, and `--group-aggregation-paths` when they only summarize command
