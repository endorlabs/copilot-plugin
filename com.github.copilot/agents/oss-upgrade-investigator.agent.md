...
- `project-by-git`/evidence-check: `endorctl agent api --agent-id oss-upgrade-investigator list -r Project -n <namespace> --filter 'spec.git.full_name=="<owner/repo-lowercased>"' --page-size 2 --field-mask "uuid,meta.name,meta.parent_uuid,spec.git" -o json`
...
- `selected-source-usage`/explain: the host's text-search tool for `<PACKAGE_NAME>` and `<IMPORT_OR_SYMBOL>` under `<SELECTED_MANIFEST_OR_SOURCE_DIR>`, reporting file and line. Do not shell out to `rg`, `grep`, or `Select-String` for this.
